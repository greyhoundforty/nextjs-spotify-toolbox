...
exports.handler = async (req, res) => {
  ...
  const { errors, data } = await NetlifyGraph.fetchGetLatestTracks({  }, {accessToken: accessToken}); 
...
/**
async function fetchGetLatestTracks(params) {
  const {} = params || {};
  const resp = await fetch(`/api/GetLatestTracks?`, {
    method: "GET"
  });
